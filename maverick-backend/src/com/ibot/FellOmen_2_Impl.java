 ...
 * @author Ian Fellows
 */
public class FellOmen_2_Impl
{
	private int numRaises = 1;

	public FellOmen_2_Impl() { }  

	public void newRound (int stage) {
	 ...
	 */
	public Action preFlopAction(MaverickGameInfo gi, Card c1, Card c2, int ourSeat) 
	{
		...
		if(rand<foldProb)
			...
			return Iface.checkOrCall(gi, (int) gi.getBetAmount());
		else if(rand<(foldProb+callProb+raiseProb))
			return Iface.betOrRaise(gi, (int) (gi.getBetAmount()+gi.getBetSize()));
		return (response);
	}               


	public Action flopAction(MaverickGameInfo gi, double handRank, double potential, double negPotential,int flopBDindex, int ourSeat , int preflopHist)
	{
		Action response = null;
		String player;
		if(!( gi.getCurrentPlayerSeat()==gi.getButtonSeat() )) {
	...
	public Action turnAction(MaverickGameInfo gi, double handRank, double potential, double negPotential,int flopBDindex, int ourSeat , int preflopHist , int flopHist)
	{
		Action response = null;
		String player;
		if(!( gi.getCurrentPlayerSeat()==gi.getButtonSeat() )) {
	...
	public Action riverAction(MaverickGameInfo gi , double handRank ,int flopBDindex, int ourSeat , int preflopHist , int flopHist , int turnHist)
	{
		Action response = null;
		String player;
		if(!( gi.getCurrentPlayerSeat()==gi.getButtonSeat() )) {
...
//////////////////////////////////////////////////////////*/
	public int getFlopBoardIndex(String sBoard)
	{
		...
		if(board.size()!=3) debug("getFlopBoardIndex: Board size = "+board.size());
		...
		if(i>4) debug("Invalid flop weight:"+flopWt);
		//If all same suit//
	...
	protected double[][] getActionTable(int round, String fileName)
	{
		...
		while(k<8)
		{
			...
			try
			{
				String path = "Strategy_FellOmen_2/"+k+".zip";
				debug("trying "+path);
				jf= new JarFile(path);
