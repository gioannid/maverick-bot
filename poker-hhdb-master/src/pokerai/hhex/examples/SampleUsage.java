...
public class SampleUsage {
  ...
  public static void main(String[] args) {
    // ---
    File dir = new File(Consts.rootFolder);
    String[] all = dir.list();
    // Scan for all files from site PS, Holde NL, 9-seats, and agregate stats for all found DBs
    time = System.currentTimeMillis();
    for (int i = 0; i < all.length; i++) if (all[i].startsWith("pokerai.org.sample" + Consts.SITE_PS + "_" + Consts.HOLDEM_FL + "_6") && all[i].endsWith(".hhex")) {
      scan(Consts.rootFolder, all[i]);
    }
    printStats();
