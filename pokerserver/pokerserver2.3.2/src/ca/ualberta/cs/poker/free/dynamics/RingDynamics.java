...
public class RingDynamics {
    ...
    public StringBuilder[] playerBetting;
    ...
    public boolean[] potWinners;
    public int numWinners = -1;
    public boolean reverseBlindsHU = true;
    ...
    public RingDynamics (RingDynamics other) {
    	readyToEndHand = other.readyToEndHand;
    	info = other.info; // shallow copy
        numPlayers = other.numPlayers;
        numSeats = other.numSeats;
        player = other.player; // shallow copy
        if (other.stack != null)
        	stack = other.stack.clone();
        botNames = other.botNames; // shallow copy
        if (other.inPot != null)
        	inPot = other.inPot.clone();
        if (other.amountWon != null)
        	amountWon = other.amountWon.clone();
        if (other.grossWon != null)
        	grossWon = other.grossWon.clone();
        if (other.additionalWon != null)
        	additionalWon = other.additionalWon.clone();
        if (other.active != null)
        	active = other.active.clone();
        numActivePlayers = other.numActivePlayers;
        if (other.canRaiseNextTurn != null)
        	canRaiseNextTurn = other.canRaiseNextTurn.clone();
        roundBets = other.roundBets;
        roundCalls = other.roundCalls;
        bettingSequence = other.bettingSequence;
        if (other.playerBetting != null) {
            playerBetting = new StringBuilder[other.playerBetting.length];
            for (int i = 0; i < playerBetting.length; i++)
            	playerBetting[i] = new StringBuilder(other.playerBetting[i]);
        }
        seatToAct = other.seatToAct;
        lastActionSeat = other.lastActionSeat;
        lastBetSize = other.lastBetSize;
        roundIndex = other.roundIndex;
        firstActionOnRound = other.firstActionOnRound;
        lastPartialBetSize = other.lastPartialBetSize;
        handOver = other.handOver;
        hole = other.hole;	// very shallow copy
        board = other.board; // very shallow copy
        handNumber = other.handNumber;
        if (other.potWinners != null)
        	potWinners = other.potWinners.clone();
        numWinners = other.numWinners;
        reverseBlindsHU = other.reverseBlindsHU;
    }
    ...
    public void dealCards(BufferedReader cardFile){
		...
		for (int seat = 0; seat < numSeats; seat++) {
			...
			int shiftsLeft = (int) (handNumber % numPlayers);
			...
			hole[seat][0] = Card.get(cards.substring(4 * canonicalSeatIndex, 4 * canonicalSeatIndex + 2));
			hole[seat][1] = Card.get(cards.substring(4 * canonicalSeatIndex + 2, 4 * canonicalSeatIndex + 4));
		}

		int boardBegin = 4 * numPlayers;
		for (int i = 0; i < 5; i++) {
			board[i] = Card.get(cards.substring(boardBegin + (2 * i), boardBegin + 2 + (2 * i)));
		}
    ...
    public void initializeBets(){
      bettingSequence = "";
      playerBetting = new StringBuilder[numSeats];
      readyToEndHand = false;
      handOver = false;
      amountWon = null;
      grossWon = null;
      //winner = null;
    	...
    	switch (roundIndex) {
    		case 0:
    			if ((board[0] == null) || (board[1] == null) || (board[2] == null)) {
    				System.err.println("Postponed flop because of missing cards");
    				return;
    			}
    			break;
    		case 1:
    			if (board[3] == null) {
       				System.err.println("Postponed turn because of missing cards");
    				return;
    			}
    			break;
    		case 2:
    			if (board[4] == null) {
       				System.err.println("Postponed river because of missing cards");
    				return;
    			}
    			break;
    	}
...
/*        		System.out.println("Delaying end of hand...")*/;
        }
    }
    ...
    public String getCardState(int seat) {
		...
		for (int i = 0; i < numSeats; i++) {
			...
			if ((isSeatVisible(seat, i) || seat == -1) 
					/*MOD maverick BEGIN*/&& (hole[i].length > 0)/*MOD maverick END*/) {
    ...
    public void handleCall(){
      ...
      roundCalls++;
      if (amountToCall > 0)
    	  playerBetting[seatToAct].append('c');
      else
    	  playerBetting[seatToAct].append('k');
      int amount = amountToCall+inPot[seatToAct];
      if (isBettingSequenceComplex()){
    	  bettingSequence += amount;
    	  playerBetting[seatToAct].append(amount);
      }
    ...
    public void handleRaise(int amount){
    	...
    	bettingSequence +=  'r';
    	if (roundBets == 0)
    		playerBetting[seatToAct].append('b');
    	else
    		playerBetting[seatToAct].append('r');
    	if (isBettingSequenceComplex()){
    	  bettingSequence += amount;
    	  playerBetting[seatToAct].append(amount);
    	}
    ...
    public void handleFold(){
       ...
       playerBetting[seatToAct].append('f');
       firstActionOnRound = false;
...
/*    		   System.out.println("Delaying end of hand (after fold)...")*/;
       } else {
         if (getNumPlayersLeftToAct()==0){
    	     incrementRound();
         } else {
    ...
    public void handleSidePot(boolean[] playersIn, int potSize){
        potWinners = new boolean[numSeats];
        for(int i=0;i<numSeats;i++){
            potWinners[i]=false;
          }
    ...
    public void endHand(){
        ...
        for(int i=0;i<numSeats;i++){
        	if (additionalWon != null)
        		amountWon[i]=additionalWon[i]-inPot[i];
        	else
    ...
    public boolean isGameOver(){
    	return (roundIndex==4)||(numActivePlayers==1);
    }
    ...
    public Card[] getBoard (int round) {
		Card[] rb = new Card[5];
		if (round >= 1) {
			rb[0] = board[0];
			rb[1] = board[1];
    ...
    public Card[] getBoard (boolean revealedOnly) {
    	if (! revealedOnly) {
    		return board;
    	} else {
     		return getBoard (roundIndex);
    	}
    }
    
    public void setBoard (Card[] b) {
    	board = b;
    }
    ...
    public String toString() {
    	char NEWLINE = '\n';
    	StringBuffer result = new StringBuffer("type: ").append(info.limitGame).
    			append(info.stackBoundGame ? " STACKBOUND\n":"\n");
    	result.append("bettingSequence: ").append(bettingSequence).append(NEWLINE);
    	result.append("lastActionSeat: ").append(lastActionSeat).append(NEWLINE);
    	result.append("seatToAct: ").append(seatToAct).append(NEWLINE);
    	result.append("roundBets: ").append(roundBets).append(NEWLINE);
    	
    	result.append("players names (by seat):");
    	for(int s=0;s<active.length;s++){
    		result.append(' ').append(botNames[seatToPlayer(s)]);
    	}
    	
    	result.append("\nactive (by seat):");
    	for(int i=0;i<active.length;i++){
    		result.append(' ').append(active[i]);
    	}
    	
    	result.append("\ninPot (by seat):");
    	for(int i=0;i<inPot.length;i++){
    		result.append(' ').append(inPot[i]);
    	}
    	
    	result.append("\nstack (by seat):");
    	for(int i=0;i<stack.length;i++){
    		result.append(' ').append(stack[seatToPlayer(i)]);
    	}
    	
    	result.append("\ncanRaise (by seat):");
    	for(int i=0;i<this.canRaiseNextTurn.length;i++){
    		result.append(' ').append(canRaiseNextTurn[i]);
    	}
    	
    	if (grossWon!=null){
    		result.append("\ngross won:");
    		for(int i=0;i<grossWon.length;i++){
    			result.append(' ').append(grossWon[i]);
    		}
    	}
